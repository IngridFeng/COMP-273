...
array: 	.align 2
	.space 2048		#initialize an array
...
begintran:
	addi $t0, $zero, 0	# row index - 1
	addi $t1, $zero, 1	# column index
	add $t4, $zero, $a1	# index for newbuffer
	
loopt:	mul $t2, $t0, 24	# (row index - 1) * num of columns
...
ascii:	addi $t3, $t3, 48	# change to ASCII
	...
	bne $t0, 6, next	# if it's not the last entry in this column, go to next number
	beq $t1, 24, expro	# it's the last entry in the array, exit loop 
	
	addi $t3, $zero, 10	# store newline character
	sb $t3, ($t4)
	addi $t4, $t4, 1
	
	addi $t0, $zero, 0	# update row index
	addi $t1, $t1, 1	# update column index
	j loopt
